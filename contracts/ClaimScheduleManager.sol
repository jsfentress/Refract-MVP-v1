...
contract ClaimScheduleManager {
    ...
    mapping(uint256 => ClaimSchedule) private schedules;
    ...
    function getSchedule(uint256 tokenId) external view returns (uint256[] memory, uint256[] memory) {
    ClaimSchedule storage schedule = schedules[tokenId];
    return (schedule.unlockTimestamps, schedule.unlockPercents);
}
}
